...
const findRestaurant = (list1, list2) => {
    ...
    list1.map((l, index) => list1Hash[l] = index)

    for (let [i, item] of list2.entries()) {
        if (item in list1Hash) {
            commonList[item] = list1Hash[item] + i
            min = Math.min(min, commonList[item])
        }
    }
    return list2.filter(item => commonList[item] === min)
}
...
//         if (item in list1Hash) {
//             commonList[item] = list1Hash[item] + i
//             if (commonList[item] < min) {
//                 min = commonList[item]
//                 result = item
//             } else if (commonList[item] === min) results.push(item)
//         }
//     }
//     results.push(result)
//     return results
